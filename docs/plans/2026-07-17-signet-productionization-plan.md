...
| AC-05 | Release artifacts and provenance | `uv lock --check && uv run ruff check . && uv run mypy && uv run pytest -q --cov=signet --cov-report=term-missing --cov-fail-under=85 && uv build --no-sources` | Wheels, sdist, SBOM, and provenance are reproducible and platform-specific; stable publish uses trusted publishing only. |
