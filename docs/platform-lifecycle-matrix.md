...
| Retention | logs/cache/staging/backups | Launchd logs atomically rename without truncating active writers and fail closed rather than overwrite a live rotation; cache prunes oldest-first at 1 GiB, staging admission stays at 50 MiB, and backups refuse growth above 8 GiB | `tests/test_storage_lifecycle.py`, `tests/test_staging.py`, `tests/test_lifecycle_plans.py` |
